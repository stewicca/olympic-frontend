<template>
  <header class="w-full h-56 md:h-[400px] bg-gray-100 bg-contain bg-center bg-no-repeat" :style="{'background-image': `url('https://asset.olympicfurniture.co.id/NEWS/${detail.data.image}')`}"></header>

  <div class="flex lg:grid grid-cols-12 flex-col-reverse w-full lg:w-[90%] xl:w-[70%] mx-auto">

    <!-- Share -->
    <div class="lg:relative col-span-1 mt-1 lg:mt-28">
      <div class="flex lg:sticky top-36 flex-col items-center">
        <p class="text-imperialBlue md:text-lg font-bold">Share</p>
        <ul class="flex lg:flex-col items-center gap-2 w-fit">
          <li v-for="content in shareData" :key="content.id" class="text-imperialBlue text-opacity-90 hover:text-opacity-100">
    ...
    <article class="col-span-10">
      ...
      <!-- Breadcrumb -->
      <div class="layout-article space-y-5 pb-8 pt-5 lg:pt-10 bg-gray-50">
        <div class="flex items-center justify-center gap-2">
          ...
          <NuxtLink to="/blog" class="text-imperialBlue">Blog</NuxtLink>
          <svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" class="w-3">
            <path d="M13 6L19 12L13 18M6 6L12 12L6 18" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
          </svg>
          <NuxtLink :to="'/blog/cat/' + formatTitle(detail.data.r_uni_news_categories[0].name_id)" class="text-imperialBlue">{{ detail.data.r_uni_news_categories[0].name_id }}</NuxtLink>
        </div>

        <!-- Title -->
        <h2 class="px-4 text-2xl md:text-3xl lg:text-4xl text-center font-extrabold">{{ detail.data.title }}</h2>

        <!-- Author & Date -->
        <p class="text-sm md:text-base text-center font-bold">{{ detail.data.seo_2 }} · {{ detail.data.date_indo }}</p>
        
        <BlogProduct :products="product" />

        <!-- Mini banner -->
        <NuxtLink :to="banner.data.external_link">
          <NuxtImg loading="lazy" :src="'https://asset.olympicfurniture.co.id/' + banner.data.image_desktop" alt="" class="h-auto w-full mt-4 px-4"/>
        </NuxtLink>

        <!-- Content -->
        <div id="article-content" v-html="detail.data.content" class="px-4"></div>

        <!-- Tags -->
        <div class="flex flex-wrap gap-3 px-4">
          <NuxtLink v-for="data in detail.data.r_uni_news_tags" :to="'/blog/tag/' + data.name" :key="data.id" class="px-2 py-1 text-white text-xs md:text-sm bg-imperialBlue bg-opacity-90 hover:bg-opacity-100 rounded">#{{ data.name }}</NuxtLink>
        </div>
      </div>

      <!-- Detail author -->
      <div class="layout-article col-span-10 space-y-5 mt-5 pb-8 pt-5 lg:pt-10 bg-gray-50">
        <div class="group flex gap-2 px-4">
          <div class="my-auto flex-none">
            <NuxtImg loading="lazy" :src="detail.data.author_info.image" alt="" width="300" height="300" class="w-24 h-24 mx-auto rounded-lg" />
          </div>
          <div class="flex flex-col content-between justify-center">
            <span class="capitalize">{{ detail.data.author_info.position }}</span>
            <NuxtLink :to="'/author/' + detail.data.author_info.slug" class="hover:underline">
              <span class="text-lg font-bold">{{ detail.data.author_info.name }}</span>
            </NuxtLink>
            <span class="pb-2">{{ detail.data.author_info.desc }}</span>
            <hr class="w-1/4 group-hover:w-full border-imperialBlue border-t-2 rounded-full transition-all duration-500" />
            <span class="pt-2"><strong>Editor :</strong> {{ detail.data.seo_4 }}</span>
          </div>
        </div>
      </div>
    </article>
  ...
  <BlogRelated :category="detail.data.r_uni_news_categories[0].name_id" />
</template>
...
const { data: detail } = await useFetch("/uni/news/details", {
...
const { data: product } = await useFetch(
  `/brands/product/news/${route.params.slug}`,
  {
    baseURL: config.public.api,
    query: {
      application: 2,
...
const { data: banner } = await useFetch("/brands/news/banner", {
  baseURL: config.public.api,
  query: {
    application: 2,
  },
...
const shareData = [
  ...
  {
    id: 2,
    to: `https://api.whatsapp.com/send/?phone&text=${detail.value.data.title_meta} - - https://olympicfurniture.co.id${route.fullPath}`,
...
useHead({
  title: detail.value.data.title_meta,
  meta: [
    { name: "author", content: detail.value.data.seo_2 },
    { name: "description", content: detail.value.data.description_meta },
    { name: "keywords", content: detail.value.data.keyword },
    { name: "robots", content: "all" },
    { property: "og:locale", content: "id" },
    { property: "og:title", content: detail.value.data.title_meta },
    { property: "og:description", content: detail.value.data.description_meta },
    { property: "og:type", content: "Website" },
    { property: "og:url", content: `https://olympicfurniture.co.id${route.fullPath}` },
    { property: "og:image", content: "https://asset.olympicfurniture.co.id/NEWS/" + detail.value.data.image },
    { name: "twitter:creator", content: "@OlympicFurnitur" },
    { name: "twitter:title", content: detail.value.data.title_meta },
    { name: "twitter:description", content: detail.value.data.description_meta },
    { name: "twitter:site", content: "@OlympicFurnitur" },
    { name: "twitter:image", content: "https://asset.olympicfurniture.co.id/NEWS/" + detail.value.data.image },
  ],
  htmlAttrs: { lang: "id" },
  link: [{ rel: "canonical", href: `https://olympicfuniture.co.id${route.fullPath}` }],
});
</script>
